...
def fit_glyph_plus(glyph, em, percent, wavg, havg):
    ...
    if not gwidth or not gheight:
        return
    rate = (
        (em - min(wavg - gwidth, havg - gheight) * 0.01)
        * percent
        / max(gwidth, gheight)
    )
    if gwidth < 0.95 * wavg and gheight < 0.95 * havg:
        rate = (
            (em - min(wavg - gwidth, havg - gheight) * 0.6)
            * percent
            / max(gwidth, gheight)
        )
...
def ScaleToEm(percent, afont):
    ...
    lbearing = int(em * 0.05)
    if not percent:
        ret = fontforge.askString(
            "Scale Percent?", "Percent of Line Height (em)?", "95"
        )
        if not ret:
            return
        try:
...
def YOffset(percent, afont):
    ...
    em = afont.ascent + afont.descent / 2
    if not percent:
        ret = fontforge.askString("Offset Percent?", "Percent of Offset?", "64")
        if not ret:
            return
        try:
            percent = float(ret) / 100
...
def GetSelectedBound(junk, afont):
    ...
    em = afont.ascent + afont.descent
    msg = (
        "The max width and height of the selected glyphs are:\n"
...
if fontforge.hasUserInterface():
    fontforge.registerMenuItem(
        GetSelectedBound, None, None, "Font", None, "Metrics", "Max Selected Size"
    )
    fontforge.registerMenuItem(
        CenterHeight, None, None, "Font", None, "Metrics", "Center in Height"
    )
    fontforge.registerMenuItem(
        CenterGlyph, None, None, "Font", None, "Metrics", "Center in Glyph"
    )
    fontforge.registerMenuItem(YOffset, None, None, "Font", None, "Metrics", "Y Offset")
    fontforge.registerMenuItem(
        BoundToSquare, None, None, "Font", None, "Metrics", "Bounding to Square"
    )
    fontforge.registerMenuItem(
        ScaleToEm, None, None, "Font", None, "Transform", "Scale to Em"
    )
    fontforge.registerMenuItem(
        ScaleToSquare, None, None, "Font", None, "Transform", "Scale to Sqare"
    )
    fontforge.registerMenuItem(
